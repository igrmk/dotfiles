...
map('x', 'p', 'P', { desc = 'Paste without yanking the replaced text' })
map('n', 'ZE', '<cmd>cquit<cr>', { desc = 'Quit with a non-zero exit code' })
...
-- losing GCC-only headers like quadmath.h; a missing include is fatal and kills the whole TU.
local gcc_config_homes = {}

local function gcc_config_home(compilation_database_dir)
    local cached = gcc_config_homes[compilation_database_dir]
    ...
    local include = vim.trim(probe.stdout or '')
    if probe.code ~= 0 or not vim.uv.fs_stat(include) then return nil end

    -- Conan and cross builds give unrelated toolchains the same driver name,
    -- so key on the resolved dir; it alone decides the content, making sharing safe.
    local key = vim.fn.fnamemodify(driver, ':t') .. '-' .. vim.fn.sha256(include):sub(1, 8)
    local home = vim.fn.stdpath('cache') .. '/clangd/' .. key
    local dir = home .. '/clangd'
    -- A racing nvim makes mkdir throw even with "p"; only the dir existing afterwards matters.
    pcall(vim.fn.mkdir, dir, 'p')
    if not vim.uv.fs_stat(dir) then return nil end

    -- -idirafter searches the dir last, so clang's own intrinsics still win.
    local tmp = dir .. '/config.yaml.' .. vim.fn.getpid()
    vim.fn.writefile({
        '# Generated by init.lua; edits are overwritten.',
        'CompileFlags:',
        '  Add:',
        '    - -idirafter' .. include,
    }, tmp)
    -- Rename is atomic, so a concurrent nvim never reads a half-written config.
    vim.uv.fs_rename(tmp, dir .. '/config.yaml')
    gcc_config_homes[compilation_database_dir] = home
    return home
end
